...
export class QdrantInitService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Qdrant collection "${collection}" created (dimension=${dimension}, distance=Cosine)`);
    } catch (err) {
      this.logger.warn(`Failed to initialise Qdrant collection "${collection}": ${err}`);
    }
  }
}
