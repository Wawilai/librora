...
//
// Mirrored in backend/apps/worker/src/common/billing-period.util.ts —
...
// Next reset date for display purposes (e.g. "resets Aug 15").
export function nextBillingReset(startedAt: Date, now: Date = new Date()): Date {
  const anchorDay = startedAt.getUTCDate();
  const cycleStart = new Date(currentBillingPeriod(startedAt, now));
  return clampedUtcDate(cycleStart.getUTCFullYear(), cycleStart.getUTCMonth() + 1, anchorDay);
}
